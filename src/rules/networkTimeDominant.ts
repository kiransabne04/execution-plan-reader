...
//
// Episode 34, Story 34.2 — "data movement" enrichment, not a new rule.
// `network.bytesSent` (Episode 33's `NetworkInfo`, from `network.network_bytes`)
// didn't exist as a captured field when this rule was first written — the
// finding could previously only state a PERCENTAGE of time, never how much
// data actually moved. When the byte figure is present on this same node,
// it's now included as concrete evidence alongside the existing
// percentage-based reasoning, same "enrich the existing finding with a new
// concrete fact" pattern Story 32.1 established for `explodingJoin.ts`.
...
export const networkTimeDominant: Rule = (node) => {
  ...
  const bytesSent = node.network?.bytesSent
  const hasBytes = bytesSent !== undefined && Number.isFinite(bytesSent) && bytesSent > 0
  const bytesText = hasBytes ? formatBytesCompact(bytesSent) : undefined
  const bytesNote = hasBytes ? ` Concretely, this operator moved ${bytesText} of data over the network.` : ""

  return [
    {
      ruleId: "network-time-dominant",
      severity,
      shortText: `${networkPct.toFixed(1)}% of this operator's time went to network communication${hasBytes ? ` (${bytesText} moved)` : ""} — a meaningful share of the whole query.`,
      longText:
        ...
        `the query's total — this isn't a tiny operator with a high percentage of almost nothing.${bytesNote} Possible causes ` +
      ...
      provenance: {
        ...
        computed: `${networkPct.toFixed(1)}% network, ${tb!.overallPercentage!.toFixed(1)}% of query${dominant ? `, dominant category: ${dominant.category}` : ""}${hasBytes ? `, ${bytesText} moved` : ""}`,
      },
    },
  ]
}
