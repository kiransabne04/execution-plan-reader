...
export const badRowEstimate: Rule = (node) => {
  ...
  const factorText = Number.isFinite(factor) ? `${Math.round(factor).toLocaleString()}x` : "far"

  return [
    {
      ruleId: "bad-row-estimate",
      severity: "warning",
      shortText: `Estimate off by ${factorText}: expected ${estimatedRows.toLocaleString()} rows, got ${actualRows.toLocaleString()}.`,
      longText:
        `The planner estimated ${estimatedRows.toLocaleString()} rows for this ${node.rawOperatorLabel} but ` +
        `${actualRows.toLocaleString()} actually came out — ${factorText} ${direction} than expected. Bad estimates ` +
