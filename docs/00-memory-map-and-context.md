...
| UI redesign spec — dark-only palette, app shell, node encoding, edge rendering, and five new surfaces (file drop, search palette, guided walkthrough, PNG export, content stack) | `12-ui-redesign-spec.md` — implemented as Episode 18, on branch `new-ui` |
...
- **`PlanNode` has no normalized relation/table-name field.** The field catalog promoted `predicate`/`index`/`join`/`io`/`spill`/`pruning`/`parallel`/`timeBreakdown`, but never a relation-identity field — nothing needed one before Episode 14's node matching (`src/comparison/matchNodes.ts`), which reads it straight from each engine's raw `attributes` bag instead (`"Relation Name"` / `"Object.Table"` / `"attr.table_name"`, documented in-file). Also note `index.name` is only populated by the SQL Server parser today — Postgres never promotes it onto `PlanNode.index`, only into `attributes["Index Name"]`. If a second consumer needs either of these, promoting a real normalized field (and updating both parsers + the field catalog) is worth doing then, rather than growing more attribute-key readers.
- **UI redesign complete: all 14 stories of Episode 18 shipped on branch `new-ui`** (spec `docs/12-ui-redesign-spec.md`, status confirmed) — dark-only, one consolidated token palette; every `prefers-color-scheme` branch across `src/`'s CSS was deleted, not overridden. `new-ui` has NOT yet been merged to `main` — that's the next real step for this branch, not a new story. The spec's own §8 ("parked — plan comparison") was written without knowledge that Episode 14 already shipped a working comparison feature (`src/comparison/`, `src/graph/comparison/`) — Story 18.14 restyled that existing UI onto the new shell, it did not build spec §8's "full-screen modal" concept from scratch. If a future session finds spec §8 and Episode 14 seeming to describe two different features, they're the same feature at two different points in time; Episode 14 is the one that actually shipped. Several stories found and documented genuine spec-internal contradictions (e.g. 18.12's mobile breakpoint named three different values — 620/900/480 — across the spec's own breakpoint table vs. its prose; resolved in favor of the structured table) — check `docs/BACKLOG-STATUS.md`'s Episode 18 rows for the full account of each, not just this summary.
- **Episode 19 (still on `new-ui`) retired the pre-shell hero landing page: the three-column shell is now the app's only page, from first load.** User-directed, from a screenshot of the redesign's own "2a" mockup with the explicit instruction "let this be only page for the app now." Plan Input moved permanently into the left rail, above Findings; the empty-state centre shows a plain placeholder, confirmed via `AskUserQuestion` ("Same shell, empty centre"). **This deliberately supersedes Episode 8 Story 8.1's "hero above the fold, no loading gate" AC and spec §7's matching hero-copy constraint** — both written before the shell existed, when the default view was a full-page paste form. `positioningCopy.ts`'s hero copy exports are untouched but no longer rendered anywhere; a future first-time-visitor-credibility pass should start from that file. See `docs/08-episodes-and-stories.md`'s Episode 19 header and `docs/BACKLOG-STATUS.md`'s Story 19.1 row for the full account, including the narrow-shell rule that Plan Input (unlike Findings) is never hidden by the tab switch below 860px.
