...
- **UI redesign confirmed and in progress: dark-only, one consolidated token palette, on branch `new-ui` (Episode 18, spec `docs/12-ui-redesign-spec.md`).** Every `prefers-color-scheme` branch across `src/`'s CSS is being deleted, not just overridden — a light-mode fallback is not a smaller/safer version of that work. The spec's own §8 ("parked — plan comparison") was written without knowledge that Episode 14 already shipped a working comparison feature (`src/comparison/`, `src/graph/comparison/`) — Story 18.14 restyles that existing UI onto the new shell, it does not build spec §8's "full-screen modal" concept from scratch. If a future session finds spec §8 and Episode 14 seeming to describe two different features, they're the same feature at two different points in time; Episode 14 is the one that actually shipped.
