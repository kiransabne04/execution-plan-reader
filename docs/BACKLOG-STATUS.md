...
| Story | Status | Notes |
|---|---|---|
| 22.1 — Maximize-to-viewport canvas mode | not started | A `position: fixed` CSS overlay (matching `WalkthroughOverlay`'s own established pattern), deliberately NOT the browser's real Fullscreen API — see the episode's own feasibility note for why. Needs an explicit, tested decision on Escape-key stacking order against `WalkthroughOverlay`/`SearchPalette`, and on what stays reachable (statement tabs, Findings) while maximized |
