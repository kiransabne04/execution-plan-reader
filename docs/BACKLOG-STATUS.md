...
| Story | Status | Notes |
|---|---|---|
| 22.1 — Maximize-to-viewport canvas mode | not started | A `position: fixed` CSS overlay filling the whole browser viewport (matching `WalkthroughOverlay`'s own established pattern), deliberately NOT the browser's real Fullscreen API — see the episode's own feasibility note for why. Confirmed: Findings/Beginner-Expert/Walk-me-through all stay reachable while maximized; multi-statement switching uses a new compact dropdown, not the full tab strip. Still needs an explicit, tested decision on Escape-key stacking order against `WalkthroughOverlay`/`SearchPalette` |
