...
| 22.2 — Node-click detail popup anchored at the node's position (DOM/SVG mode) | not started | Reuses `DetailPanel` via a new `variant="popup"` (Story 18.2's own established variant pattern) positioned via React Flow's `flowToScreenPosition()` — no new coordinate math needed in this mode. Needs a NEW, unit-tested viewport-edge clamp/flip function this codebase doesn't have yet. Scoped to maximized mode only — normal-mode node-click behavior is unchanged |
