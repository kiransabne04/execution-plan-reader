...
| 22.2 — Node-click detail popup anchored at the node's position (DOM/SVG mode) | done | `DetailPanel` gained `variant="popup"` + a `position` prop; `PlanGraph.tsx` gained `nodeDetailVariant` ("panel" default, "popup") and a `NodeDetailPopup` sub-component (isolated so `useViewport()`'s re-render-on-pan/zoom subscription doesn't affect the whole graph) that computes the anchor via `flowToScreenPosition()` + the new pure `computePopupPosition` (`popupPosition.ts`, 9 unit tests covering all edges/corners/degenerate sizes). Live-verified in a real browser: popup opens beside the clicked node, live-repositions and clamps correctly while panning, closes on Escape/close-button/clicking a different node. Canvas mode (300+ nodes) keeps Story 22.1's own interim overlay fallback until Story 22.3 gives it a real popup — `PlanReaderPage.tsx` picks between them via the exported `CANVAS_NODE_COUNT_THRESHOLD` |
| 22.3 — Node-click detail popup anchored at the node's position (Canvas-rendering mode) | not started | The case that actually motivated this episode ("big plan" → Episode 15's 300+-node canvas threshold). Needs a new `worldToScreen()` in `viewportTransform.ts` — the missing inverse of the existing `screenToWorld()` — since canvas mode has no per-node DOM element to ask a screen position from. Reuses Story 22.2's own popup component/clamping logic entirely; this story is only about feeding it a correct position |
