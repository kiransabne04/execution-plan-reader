...
| 25.4 — Estimate error materiality | Reworks `bad-row-estimate`'s existing severity assignment (currently binary: ratio ≥10x/≤0.1x fires, always `warning`) into a materiality score combining ratio, absolute row difference (a floor — the story's own "est 1 → actual 50" example is a 50x ratio but only 49 rows, correctly non-material), the node's own share of total plan runtime (`actualTimeMs` / `context.totalActualTimeMs`, only when `hasActualData`), and whether the node *itself* is a join operator (`JOIN_OPERATOR_TYPES`, shared with `exploding-join`) — **note, deviates from this row's original plan**: the original intent was "feeds a join" checked via the node's *parent* (`PlanNode` has no parent pointer, so this would have meant threading `cardinalityPropagation.ts`'s ancestry index into the rule); shipped instead as "the node itself is a join" specifically so `severityForEstimateError` stays a single-node pure function taking no tree/ancestry argument — `rule-engine-authoring`'s "no rule gains a cross-node argument" constraint held here on purpose, at the cost of not also escalating a bad estimate on a *leaf feeding into* a join. `severityForEstimateError`'s own in-code doc comment names this tradeoff explicitly | `computeMismatchFactor` (existing, exported, already reused by the graph's own mismatch badge) is NOT changed — the ratio number shown on the badge stays exactly what it is today; only `bad-row-estimate`'s OWN severity assignment inside the rule gains the extra materiality factors, so nothing else that reads `computeMismatchFactor` is affected |
| 25.5 — Potential statistics investigation | Text-only change to `bad-row-estimate.longText` | Replaces "Stale table statistics are the most common cause." with "large estimate errors may warrant checking" statistics freshness, correlated columns, extended statistics, skewed data, predicates, and parameter values — none stated as confirmed, no single cause claimed (`skewed data` added 2026-09-07, the list's 6th item) | Direct instruction from the story: never state "Your statistics are stale" as a fact this app can't verify from one plan, and never imply the list narrows to one cause |
...
- `longText` explains: the seek found the matching keys, SQL Server repeatedly returned to fetch the rest of each row (the loop count named explicitly), that a covering index may be worth investigating, and that a wider index has a write/storage cost trade-off — never presented as a free win.
...
- **RID Lookup (2026-09-08 addition, same story/rule/ruleId — a heap table has no clustered index, so this is the same pattern's other raw label, not a separate feature)**: distinguished at the text layer only, via `node.rawOperatorLabel` (`"Key Lookup"` vs `"RID Lookup"`) — identical trigger/severity logic for both. `longText` for a RID Lookup explains heap row retrieval correctly (fetched directly from the heap by physical Row ID, not "the clustered index," since a heap has none) and suggests a covering *nonclustered* index specifically (adding a clustered index at all is named as a separate, bigger decision). Must never state or imply "a heap is always bad" — the finding is about the volume of repeated round trips, not the storage structure; a heap is presented as a legitimate, sometimes-correct choice.
