...
|---|---|---|
| Runtime | `seq-scan-on-large-table`, `high-loop-count` | `context.hasActualData` is true — both rules' real signal is about ACTUAL execution behavior (loop counts and scan cost only mean something as *runtime*, not estimate, concepts); an estimate-only plan has no loops field at all |
| Cardinality | `bad-row-estimate`, `exploding-join`, `missing-index-opportunity`, `non-sargable-predicate` | at least one node has `estimatedRows !== undefined` — the family this dimension is named for is fundamentally about estimate quality/join fan-out, which the structural rules (`missing-index-opportunity`, `non-sargable-predicate`) can judge even estimate-only |
| Memory | `disk-spill` | at least one node has `node.spill !== undefined` — the parser attempted spill detection for it (see `SpillInfo`, `normalize.ts`) regardless of whether it actually spilled |
| I/O | `buffer-cache-inefficiency` | at least one node has `io.bufferHits`/`io.bufferReads` (Postgres w/ `BUFFERS`, SQL Server logical/physical reads) OR (Snowflake) a populated `timeBreakdown` disk-I/O share |
| Parallelism | `parallel-worker-shortfall` (**new rule, Story 23.2** — doesn't exist yet) | at least one node has both `parallel.workersPlanned` and `parallel.workersLaunched` defined — currently **Postgres-only** (`extendedFields.ts`); SQL Server's `parallel` only ever carries `workersLaunched` (thread count, no "planned DOP" field captured), so this dimension will read "not enough data" on essentially every real SQL Server plan today, and always on Snowflake (no `parallel` field at all). Stated here plainly, not discovered later: this dimension will be the least-often-scored of the five |
...
- A test with a Snowflake fixture (no actual time, no buffers, no parallel field) confirming Runtime and Parallelism come back `insufficient-data` while Cardinality/I/O (via `timeBreakdown`) still score — this is the dimension table's own central claim, worth locking in with a real fixture, not just asserted in prose.
...
### Story 23.2 — `parallel-worker-shortfall` rule (Postgres)

As a polyglot backend/data engineer (Persona B), I want to know when a query asked for parallel workers but didn't get them, so I can tell "this ran serially because of a real resource shortfall" apart from "this ran serially because it never asked" — a distinction the raw `Workers Planned`/`Workers Launched` numbers already in the detail panel (`buildStatRows.ts`) don't currently call out as a *problem*.
...
- Fires only when `node.parallel?.workersPlanned !== undefined && node.parallel?.workersLaunched !== undefined && node.parallel.workersLaunched < node.parallel.workersPlanned` — i.e., Postgres only, per the field-availability note in the dimension table above; SQL Server/Snowflake nodes never have both fields populated, so this rule silently never fires there rather than needing an explicit engine check.
- Severity scales with the shortfall's severity, not just its existence — e.g. `warning` when launched ≥ half of planned, `critical` when launched is 0 or under half — mirroring how `bufferCacheInefficiency`/other threshold-scaled rules already grade severity by degree rather than firing binary.
- `shortText`/`longText` per the standard `Warning` shape (`rule-engine-authoring` skill) — the long form should name the concrete numbers ("planned 4 workers, launched 1") and the one well-known real-world cause (`max_parallel_workers`/`max_worker_processes` exhausted by other concurrent activity on the instance), without overclaiming a diagnosis this single plan can't actually confirm (no visibility into what else was running on the server at the time — a smaller, rule-scoped instance of the same honesty this codebase already applies to parameter sensitivity).

**Testing approach**
- Positive fixture: `Workers Planned` > `Workers Launched` by a critical-grade margin.
- A second positive fixture at the warning-grade boundary, to lock in the severity threshold precisely, not just "some shortfall = critical."
- Negative fixture: `Workers Planned === Workers Launched` (parallelism worked as planned) — must not fire.
- Negative fixture: neither field present (no parallelism attempted, or a non-Postgres engine) — must not fire, and must not throw on `undefined` comparisons.
- End-to-end test through `analyzePlanText` on a real Postgres JSON fixture (not just the rule function in isolation), per this session's own established precedent for every other rule in `postgresRuleTriggerScenarios.test.ts`.

**Edge cases to handle**
| Case | Why it matters | Handling |
|---|---|---|
| `workersLaunched > workersPlanned` (shouldn't happen per Postgres's own semantics, but not provably impossible across every Postgres version) | A naive `<` check already handles this correctly (rule doesn't fire), but worth a fixture proving it explicitly rather than assuming | Explicit fixture, asserts no finding |
| SQL Server's `parallel.workersLaunched`-only shape (thread count, no planned-DOP field) | The rule's own eligibility check already excludes this by construction (`workersPlanned` is `undefined`), but this is exactly the kind of engine-shape assumption worth a dedicated regression fixture, not just inferred from the type signature | SQL Server fixture with `parallel.workersLaunched` set and no `workersPlanned` — asserts no finding, and no thrown error |
| A node whose parallelism is entirely healthy sitting alongside a sibling with a real shortfall | Dedup/family logic (Story 23.1) already handles "worst instance wins" for scoring, but the FINDING itself must still be attributed to the right specific node, not the plan root | Standard per-node rule shape — no special-casing needed beyond what every other node-level rule already does |
...
| Maximized mode (Episode 22) | A new always-visible card could collide with the maximized pane's own toolbar the same way Story 22.1 found for the search/maximize toolbar | Explicit decision needed: does the card stay visible while maximized (another toolbar element, following Episode 22's established "report state outward, second render location" pattern) or is it scoped to normal mode only — pick one, state it, test it, don't leave it as whatever falls out of CSS by accident (the exact instruction Episode 22's own edge-case tables already modeled) |
