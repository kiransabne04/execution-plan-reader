...
**Implementation decision made during the build, not in the original request — disclosed here rather than silently made**: AC #4 moving Findings into a bottom drawer INSIDE the canvas retires Story 18.2/18.12's own "Findings and the graph become tabs below 860px" mechanism (`isNarrowShell`/`activeShellTab`, both removed from `PlanReaderPage.tsx`). Its entire premise was Findings and the graph competing for the SAME side-by-side space as two full-height regions; once Findings lives in a drawer INSIDE the canvas pane, the two tabs would show near-identical content (both include the graph — only the drawer's own open/closed state would differ), which stopped being a meaningful choice, and the narrow 56px icon rail no longer needs the canvas to itself the way the old wide input rail did. The underlying INTENT the tabs served on true mobile (spec §5 `1k`'s "Findings leads, not the graph") is preserved through the SAME layout-effect mechanism, now driving the findings drawer's default-open state instead of which tab is active — an improvement, not a regression: the graph is never hidden entirely the way the old either/or tab switch did.
...
| "Keep panel open" pinned | A user comparing node data continuously needs the panel to survive deselecting a node, unlike the default overlay-on-select behavior | Assert turning the pin on keeps the panel visible (grid-track, ≥1180px) after closing/deselecting, distinct from the default's disappear-on-close | `e2e/plan-shell.spec.ts`'s own "pinned detail panel" describe block — grouped with this file's other detail-panel-breakpoint coverage rather than a separate file, since it's the same component/mechanism these tests already own |
| Mobile viewport (repeats: overlay/close, icon rail, findings drawer) | A shrunk desktop layout is not automatically a usable mobile one — this app's own existing mobile commitments (Episode 18 Stories 18.12/16.2) apply here too | Re-run the equivalent of the selection-overlay, icon-rail, and findings-drawer tests at the existing true-mobile breakpoint (390×844); confirm each pattern has a sensible touch/mobile shape (e.g. the detail overlay stays the existing bottom-sheet treatment already built for narrow viewports, and the findings drawer defaults OPEN rather than needing a tab switch) rather than a shrunk, harder-to-tap desktop control | `e2e/mobile-breakpoints.spec.ts` (rewritten — the tab-switch mechanism it originally verified is retired, see this episode's own AC #4) + `e2e/mobile-viewport.spec.ts`/`mobile-cpu-throttled.spec.ts` (updated: the graph no longer needs an explicit tab click to reach) |
...
| Full regression pass | This story restructures WHERE things live, not WHAT they do — content-level tests must survive untouched | Re-run every existing detail-panel-content, graph-rendering, and findings-list-content e2e/component test; confirm all still pass inside the new layout | Every existing e2e/component suite re-run, not assumed. Found and fixed 3 real, previously-undetected layout bugs this way (a CSS Grid placement-order bug that silently shrank the canvas to 56px wide, a visually-hidden-label overflow bug that corrupted the reported viewport width under mobile emulation, and a scrim z-index bug blocking the findings drawer/icon-rail buttons) — none catchable by any unit/component test, since jsdom doesn't lay out real CSS Grid or real overflow. One PRE-EXISTING, unrelated failure found and disclosed (a focus race between `WalkthroughOverlay` and `DetailPanel`, confirmed present on `main` before this story via a temporary worktree checkout) — marked `test.fail()` with a full root-cause comment, left unfixed as out of this story's scope. Final: 1258/1258 unit/component tests, 96/96 e2e tests (the 1 pre-existing failure correctly marked, not silently skipped) passing |
...
### Story 26.7 — Pixel-accurate restyle pass against the reference mockup

Source: the user manually tested the merged Episode 26 shell against the reference mockup Episode 26's own intro names (a live, iteratively-confirmed Artifact) and reported it looked "worst" / "not as smooth" — a direct comparison against a screenshot of the mockup surfaced four real, concrete divergences the individual 26.1–26.6 stories introduced or never closed. Confirmed with the user (AskUserQuestion) on the one item that reverses an explicit, separately-documented product requirement (node color-by-metric encoding) before implementing; the other three were pre-authorized as part of the same "fix all these, pixel to pixel" instruction.

**Acceptance criteria**
- **Node cards** (`canvasDraw.ts`): the per-node metric-color heatmap fill/border is replaced with a flat, neutral card (`--pg-card-bg` fill, `--color-border-strong` border at rest, `--color-accent` border on hover) — matching the mockup's own uniform `.node` treatment exactly. Confirmed explicitly with the user: this supersedes the `graph-visualization` skill's original "node size AND color both scale with the active metric" requirement — size-by-metric is untouched, only color is removed. The "Colour [swatch] {metric}" legend row in the metrics strip is removed with it (a legend for an encoding that no longer exists); "Width = {metric}" stays. Severity ring and estimate-vs-actual mismatch's dashed border are UNCHANGED — both are separate, explicitly-documented ACs (Story 18.4) this story does not touch; mismatch's dashed border color changes from the old heat color to the same neutral/hover border color every other card edge uses (the dash shape itself, not a special color, is what already carried the "never color alone" signal). Badges (loop count, mismatch, spill, severity, comparison) gain a rounded, tinted pill background (matching the mockup's own `.node .flag` chip) instead of rendering as plain stacked text.
- **Statement tab active indicator** (`planReaderPage.css`): moves from a bottom `border-bottom` underline (Story 26.5's own choice) to a TOP 2px accent bar plus a background blend into the canvas below, matching the mockup's `.tab.active`/`.tab.active::after` exactly.
- **Icon-rail active state** (`planReaderPage.css`): Story 26.2's solid full-accent pill (999px radius, white text) is replaced with the mockup's own softer treatment — 8px radius, a 22%-accent-tinted background (`color-mix`), `--color-accent-on-tint` text.
- **Status bar** (`planReaderPage.css`): gains the mockup's own subtle glowing top edge (`box-shadow: inset 0 1px 0` in a translucent accent), additive to the existing plain `border-top`.
- **PNG export stays visually consistent with the live canvas** (`exportPng.ts`/`PlanGraph.tsx`): the flat node-card colors above are threaded through export the same way every other canvas color already is; export's own opaque page background is corrected from the card/surface color to the canvas background color (`--color-bg-canvas`) — the old choice would otherwise make every node vanish into an export background now painted the same color as the cards themselves.
- Explicitly OUT of scope, named to the user as such rather than silently attempted: the app bar, severity-opener banner, and Query Health card that render above the graph toolbar. None of these exist in the mockup (its own scope only ever covered the rail/sidebar/tabs/canvas/status-bar region), but they're real, separate, pre-Episode-26 features — restyling or removing them is a different, much larger product decision this story does not make.

**Testing approach**
- Unit: `encoding.test.ts`/`buildGraphElements.test.ts` updated for `colorFor`'s removal (not replaced — the function no longer exists). `canvasDraw.test.ts` gains coverage for the hovered-node border and the badge-pill fill call, and its existing calls/comments are updated where they referenced the old heat-color fill.
- Component: `PlanReaderPage.test.tsx` — no new test needed; the colour-legend span carried no test coverage of its own to update (grep-confirmed before removing).
- Regression: full suite, plus a real-browser live-verify (Chrome extension) comparing the restyled shell against the reference mockup side by side.

**Edge cases to handle**
| Case | Why it matters | Handling |
|---|---|---|
| A collapsed run of trivial statements (Story 20.1's own group tabs) | Shares the same tab strip as real statement tabs, but never carries `aria-selected` | Its own permanent dashed indicator moves to the same top position as the real active bar (was bottom), keeping the "represents several things, not one" language intact |
| A node carrying multiple badges at once (loop count + mismatch + severity, say) | Pills are taller than the old plain-text rows they replace | Stacking math (`drawBadge`'s own bottom-up accumulator) accounts for the new pill height, not the old text-line height |
| PNG export of a plan with severity/comparison badges | The badge pill's tinted background needs the same hex-to-rgba translucency helper the live canvas uses | `colorWithAlpha` already handles both `hsl(...)` and `#rrggbb` inputs — reused as-is, not re-derived for export |
