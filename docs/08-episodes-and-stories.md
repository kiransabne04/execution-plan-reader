...
2. **Node-anchored popup positioning** is real, non-trivial, net-new work — this app has never anchored a floating panel to a node's on-screen position before. It needs:
   ...
   - **Staying anchored during pan/zoom**: the node's screen position changes continuously while panning/zooming; the popup must either re-position every frame (matching the node) or close itself on any pan/zoom gesture. The story below picks the latter (close-on-pan/zoom) as the simpler, more conventional behavior (matching how most editor/design-tool node inspectors behave) — reposition-every-frame is flagged as a possible future refinement, not required here.
3. **Scope relative to the existing right-rail/overlay `DetailPanel`**: `DetailPanel` already has three call sites (`PlanGraph`'s own default overlay render, each `PlanComparisonView` pane, and the app shell's right-rail `"shell"` variant) and an established `variant` prop pattern for exactly this kind of "same content, different chrome" need. The node-anchored popup is a NATURAL fourth variant (`"popup"`) reusing the exact same `DetailPanel` component and content — not a new content-authoring surface, matching this codebase's own repeated insistence (walkthrough, findings, tooltip) that detail content is authored once and only ever re-chromed, never re-copied. **Decision made here, to be confirmed**: fullscreen mode's popup behavior is scoped to fullscreen mode only — the existing right-rail/overlay behavior in normal (non-fullscreen) mode is completely unchanged by this episode. This is the lowest-regression-risk interpretation of the request; making the popup the ONLY way details are ever shown (replacing the right rail everywhere, including inside `PlanComparisonView`) is a much larger, separate decision this episode deliberately does not make.
...
- Maximized mode shows: the graph itself, its existing zoom/pan controls and node-search bar (`SearchPalette`'s trigger), and — for a multi-statement batch — SOME way to switch statements without leaving maximized mode (the statement tab strip, or a compact equivalent); exact scoping of what else remains reachable (Findings, Beginner/Expert toggle, Walk-me-through) is this story's own acceptance-criteria decision, not deferred silently.
...
- The popup closes when: its own close button is clicked, Escape is pressed, the user clicks a DIFFERENT node (which reopens the popup at the new node's position — not two popups at once), or the user pans/zooms the canvas (Story 22's own feasibility note: chosen over live-repositioning-every-frame as the simpler, more predictable behavior for this first version).
...
- Component test: panning/zooming while the popup is open closes it (per the acceptance criteria's explicit choice).
...
- Popup closes on pan/drag or wheel-zoom, matching Story 22.2's own chosen behavior — canvas mode's own `handlePointerMove`/`handleWheel` are the natural places to fire that close, since they already know the instant a pan/zoom gesture starts.
...
- e2e test on a real large (300+-node) fixture, reusing `e2e/canvas-large-plan.spec.ts`'s established real-browser pixel-painting verification pattern: click a node, confirm a popup actually renders at a position near that node on screen (not just that SOME popup opened somewhere).
