...
**Recommendation: React Flow with dagre for layout, not raw D3 or Mermaid** — for plans below the canvas-rendering threshold defined below. See the revision note immediately following for large plans.

**Revision (based on real browser performance testing — Episode 15 in `docs/08-episodes-and-stories.md`)**: manual testing surfaced real responsiveness problems on large plans, which is a well-documented, structural limitation of React Flow's rendering approach, not a bug to optimize away — React Flow renders one DOM element per node, and DOM/SVG-based rendering degrades as node count grows, while canvas rendering draws directly to a pixel surface with near-constant performance regardless of node count, at the cost of losing built-in interactivity and accessibility (both of which then have to be rebuilt deliberately — see the new `canvas-rendering-performance` skill). **Decision: hybrid rendering — React Flow/DOM/SVG remains the path for plans below a node-count threshold (most plans, and where its interactivity/accessibility advantages matter most), canvas rendering (dagre still computes layout; a custom canvas renderer draws it, with manual hit-testing for interaction) takes over above that threshold.** This is additive to, not a replacement of, the original recommendation below — the reasoning for React Flow below the threshold is unchanged.
...
**A backend-free alternative worth preferring as the default**: rather than server-published links being the only sharing mechanism, encoding the plan state directly into a URL fragment (never sent in any HTTP request, unlike a query parameter) lets sharing stay within the same fully-client-side guarantee as the rest of the tool — see Episode 11 Story 11.2 in `docs/08-episodes-and-stories.md` for the full design, including the honest handling required when a plan is too large to fit in a URL. The two approaches aren't mutually exclusive: fragment-encoding as the default, server-published links (Story 11.1) as a fallback for plans that don't fit.
